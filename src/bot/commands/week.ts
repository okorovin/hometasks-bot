...
export async function weekCommand(ctx: Context): Promise<void> {
    ...
    const tasks = await taskService.getWeek(user.id, user.timezone)

    if (tasks.length === 0) {
        await ctx.reply("📋 No tasks for the next 7 days.")
        return
    }
    ...
    const text = `📅 <b>This week</b> (${total} tasks):\n\n${lines.join("\n")}`
    const kb = taskListKeyboard(items, "week", page, totalPages, pageOffset)
